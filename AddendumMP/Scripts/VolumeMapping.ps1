...
#
# (c) Copyright 2010, QND srl, All Rights Reserved
# Proprietary and confidential to QND srl              
#
#*************************************************************************
...
[string]$ClusterSharedVolumeName=$(throw 'must have a value'),
[string]$FriendlyVolumeName=$(throw 'must have a value'),
...
try
{
	$parts = $FriendlyVolumeName.Split('\')
	$volume = $parts[$parts.Count-1]
    if ($volume)
    {
        $discoveryData = $g_api.CreateDiscoveryData(0, $sourceId, $managedEntityId)
		$PInstance = $discoveryData.CreateClassInstance("$MPElement[Name='QND.ClusterSharedVolume.Ext']$")
			
		$PInstance.AddProperty("$MPElement[Name='Windows!Microsoft.Windows.Computer']/PrincipalName$", $computerName)
		$PInstance.AddProperty("$MPElement[Name='ClusterDisk!Microsoft.Windows.Server.ClusterSharedVolumeMonitoring.ClusterSharedVolume']/ClusterSharedVolumeName$", $ClusterSharedVolumeName)
		$PInstance.AddProperty("$MPElement[Name='QND.ClusterSharedVolume.Ext']/VolumeId$", $volume)
