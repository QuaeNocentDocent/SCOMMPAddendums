...
try
{
	...
	foreach($pg in $allPG) {
		...
		Log-Event $START_EVENT_ID $EVENT_TYPE_SUCCESS ("Got PG $($pg.name) SLA $pgSLA hours for DPMServer: $dpmServerName on Host:$windowsServerName") $TRACE_VERBOSE
		   $pgInstance = $discoveryData.CreateClassInstance("$MPElement[Name='QND.DPM2016.SLAProtectionGroup']$")
		   $pgInstance.AddProperty("$MPElement[Name='Windows!Microsoft.Windows.Computer']/PrincipalName$", $windowsServerName)
		   $pgInstance.AddProperty("$MPElement[Name='DPM!Microsoft.SystemCenter.DataProtectionManager.2011.Library.DPMSeed']/DPMServerName$", $dpmServerName)
		   $pgInstance.AddProperty("$MPElement[Name='DPM!Microsoft.SystemCenter.DataProtectionManager.2011.Library.DPMServer']/DPMServerName$", $dpmServerName)
		   # $pgInstance.AddProperty("$MPElement[Name='DPM!Microsoft.SystemCenter.DataProtectionManager.2011.Library.ProtectionGroup']/DPMServerName$", $dpmServerName)
		   $pgInstance.AddProperty("$MPElement[Name='DPM!Microsoft.SystemCenter.DataProtectionManager.2011.Library.ProtectionGroup']/DPMObjectID$", $pg.ProtectionGroupId.ToString())
		   $pgInstance.AddProperty("$MPElement[Name='QND.DPM2016.SLAProtectionGroup']/SLA$", $pgSLA)
